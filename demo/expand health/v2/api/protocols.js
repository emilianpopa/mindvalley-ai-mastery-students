...
router.post('/generate', authenticateToken, async (req, res, next) => {
  try {
    ...
    try {
      ...
      // NOTE: Engagement plan generation is NOT automatic.
      // The user must explicitly click "Generate Engagement Plan" after reviewing the protocol.
      // This ensures proper review workflow and preserves the higher-quality manual generation process.
...
router.post('/:id/generate-engagement-plan', authenticateToken, async (req, res, next) => {
  try {
    ...
    // Try to get structured protocol data - prefer protocol_data (source of truth)
    let protocolData = null;
    try {
      // First try protocol_data column (source of truth for clinical protocol)
      if (protocol.protocol_data) {
        protocolData = typeof protocol.protocol_data === 'string'
          ? JSON.parse(protocol.protocol_data)
          : protocol.protocol_data;
        console.log('[Engagement Plan] Using protocol_data (source of truth)');
      }
      // Fallback to ai_recommendations if it has protocol structure
      ...
      // Fallback to modules if no protocol structure found
